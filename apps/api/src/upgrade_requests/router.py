...
def _row_to_out(row: dict) -> UpgradeRequestOut:
    return UpgradeRequestOut(
        ...
        requester_phone_e164=row.get("requester_phone_e164"),
        owner_email=row.get("owner_email"),
        owner_display_name=row.get("owner_display_name"),
        owner_phone_e164=row.get("owner_phone_e164"),
        status=row["status"],
        note=row["note"],
...
_LIST_SELECT = """
    SELECT
        ...
        ru.phone_e164       AS requester_phone_e164,
        ou.email            AS owner_email,
        ou.display_name     AS owner_display_name,
        ou.phone_e164       AS owner_phone_e164,
        ur.status,
        ur.note,
        ur.created_at,
        ur.resolved_at,
